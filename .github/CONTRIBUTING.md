...
If you want to contribute code to Auditbooks, please go through the following sections for tips and guidelines:
...
A few rules of thumb to ensure that you're contributing maintainable code to Auditbooks:
...
- **Rebase don't merge**: Merge commits are ugly and should be used only to
  merge a large PR.
- **Format your code**: Auditbooks uses `prettier` and `eslint` rules for code
...
We strive to make Auditbooks as easy and simple to use as possible, and
...
- **Documentation**: If the feature being added requires an explanation then
  [documentation](https://landigit.com/auditbooks/) should be updated in the
