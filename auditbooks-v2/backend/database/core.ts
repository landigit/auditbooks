...
import type { DB } from "src/renderer/src/types/db";
...
export default class DatabaseCore extends DatabaseBase {
	...
	async migrate(config: MigrationConfig = {}) {
		...
		console.log(
			`[DatabaseCore] Create: ${create.length}, Alter: ${alter.length}`,
		);
		...
		if (!hasSingleValueTable) {
			console.log(
				"[DatabaseCore] Fetching singles update list (post-create)...",
			);
			singlesConfig = await this.#getSinglesUpdateList();
		}
