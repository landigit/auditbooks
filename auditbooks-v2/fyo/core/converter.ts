...
export class Converter {
	...
	toDocValueMap(
		...
		const finalRawValueMap = rawValueMap ?? {};
		if (Array.isArray(finalRawValueMap)) {
			return finalRawValueMap.map((dv) => this.#toDocValueMap(schemaName, dv));
		}
		return this.#toDocValueMap(schemaName, finalRawValueMap);
	}
	...
	toRawValueMap(
		...
		const finalDocValueMap = docValueMap ?? {};
		if (Array.isArray(finalDocValueMap)) {
			return finalDocValueMap.map((dv) => this.#toRawValueMap(schemaName, dv));
		}
		return this.#toRawValueMap(schemaName, finalDocValueMap);
	}
...
function toDocInt(value: RawValue, field: Field): number {
	...
	if (typeof value === "string") {
		finalValue = safeParseInt(value);
	}

	return toDocFloat(finalValue, field);
}
...
function toDocFloat(value: RawValue, field: Field): number {
	...
	let finalValue = value;
	if (typeof value === "string") {
		finalValue = safeParseFloat(value);
	}
	...
	throwError(finalValue, field, "doc");
}
...
function toRawDate(value: DocValue, field: Field): string | null {
	...
	if (finalValue instanceof Date) {
		return DateTime.fromJSDate(finalValue).toISODate();
	}

	if (finalValue instanceof DateTime) {
		return finalValue.toISODate();
	}

	throwError(finalValue, field, "raw");
}
