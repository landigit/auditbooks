...
export class Doc extends Observable<DocValue | Doc[]> {
	...
	parent?: string; // Add parent property for child docs
	...
	constructor(
		...
		// biome-ignore lint/correctness/noConstructorReturn: Use reactive for Vue 3 integration
	...
	async set(
		...
		if (Array.isArray(effectiveValue)) {
			for (const row of effectiveValue) {
				this.push(fieldname, row);
			}
		} else {
			const field = this.fieldMap[fieldname];
			await this._validateField(field, effectiveValue);
			this[fieldname] = effectiveValue;
		}
	...
	async load() {
		...
		if (this.schema.isSingle && data && !data.name) {
			data.name = this.name ?? "";
		}
	...
	async delete() {
		...
		if (!this.name) throw new Error("Cannot delete document without name");
	...
	async rename(newName: string) {
		...
		if (!this.name) throw new Error("Cannot rename document without name");
