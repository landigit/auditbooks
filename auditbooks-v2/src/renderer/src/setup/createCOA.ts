...
async function getCOA(chartOfAccounts: string): Promise<COATree> {
	...
	try {
		const fixtures = import.meta.glob("../../../fixtures/verified/*.json");
		const path = `../../../fixtures/verified/${conCode}.json`;

		if (fixtures[path]) {
			const countryCoa = (await fixtures[path]()) as { default: { tree: COATree } };
			return countryCoa.default.tree;
		}
		return getStandardCOA();
	} catch (e) {
		console.error("Failed to load COA fixture:", e);
		return getStandardCOA();
	}
}
