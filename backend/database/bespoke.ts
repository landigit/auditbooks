...
export class BespokeQueries {
	...
	static async getCashflow(db: DatabaseCore, fromDate: string, toDate: string) {
		...
		return (await knex("AccountingLedgerEntry")
			...
			.whereBetween("date", [fromDate, toDate])
			.groupBy(dateAsMonthYear as any)) as Cashflow;
	}
	...
	static async getPOSTransactedAmount(
		...
		const paymentEntryNames: string[] = (
			(await knex(ModelNameEnum.PaymentFor)
		...
		const groupedAmounts = (await knex(ModelNameEnum.Payment)
		...
		for (const row of groupedAmounts) {
			const paymentRefs = (await knex(ModelNameEnum.PaymentFor)
