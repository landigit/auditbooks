...
import { getDefaultMetaFieldValueMap, SYSTEM, sqliteTypeMap } from "../helpers";
import type {
	AlterConfig,
	ColumnDiff,
	FieldValueMap,
...
export default class DatabaseCore extends DatabaseBase {
	...
	async connect() {
		...
		await (this.knex as Knex).raw("PRAGMA foreign_keys=ON");
	}
	...
	async getAll(
		...
		const hasCreated = !!(schema.fields || []).find(
			(f) => f.fieldname === "created",
		);
	...
	async #tableExists(schemaName: string): Promise<boolean> {
		return (await this.knex?.schema.hasTable(schemaName)) ?? false;
	}
	...
	async #singleExists(singleSchemaName: string): Promise<boolean> {
		...
		const res = await knex("SingleValue")
	...
	async #dropColumns(schemaName: string, targetColumns: string[]) {
		...
		await knex.schema.table(schemaName, (table) => {
	...
	async #getTableColumns(schemaName: string): Promise<string[]> {
		...
		const info: FieldValueMap[] =
			(await knex.raw(`PRAGMA table_info(${schemaName})`)) || [];
	...
	async #getForeignKeys(schemaName: string): Promise<string[]> {
		const foreignKeyList = (await this.knex?.raw(
			`PRAGMA foreign_key_list(${schemaName})`,
		)) as FieldValueMap[] | undefined;
	...
	#getQueryBuilder(
		...
		const builder = knex.select(fields).from(schemaName);
		...
		if (Array.isArray(groupBy)) {
			builder.groupBy(...(groupBy as string[]));
		}
	...
	async #addForeignKeys(schemaName: string) {
		...
		const tableRows = (await knex.select().from(schemaName)) || [];
	...
	async #getOne(schemaName: string, name: string, fields: string[]) {
		const fieldValueMap = (await this.knex
			?.select(fields)
			.from(schemaName)
			.where("name", name)
