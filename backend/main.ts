  ...
  try {
    ...
    switch (method) {
      ...
      case 'update': {
        ...
        await kv.set(['records', schemaName, record.name], {
          ...record,
          modified: new Date().toISOString(),
        });
        data = record;
      ...
      case 'getSingleValues': {
        ...
        for (const arg of args) {
          const { fieldname, parent } =
            typeof arg === 'string'
              ? { fieldname: arg, parent: 'undefined' }
              : arg;
      ...
      default:
        return c.json({
          error: {
            name: 'NotImplemented',
            message: `Method ${method} not implemented in Deno backend`,
          },
        });
    }
...
app.get('*', async (c) => {
  try {
    const html = await Deno.readTextFile(
      './dist_electron/build/src/index.html'
    );
    return c.html(html);
  } catch {
    return c.text(
      'AuditBooks Backend is running. Frontend build (dist) not found.',
      404
    );
  }
});
...
const PORT = 8080;
console.log(
  `🚀 AuditBooks Portless Backend running on http://localhost:${PORT}`
);
