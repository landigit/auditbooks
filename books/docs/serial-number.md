...
The Serial Number feature needs to be enabled first by checking the Enable Serial Number option under [Inventory Settings](settings.md)

To navigate to the list of Serial Numbers, open [Quick Search](search.md) and then type "serial number list" and select the first option.

Serialized Items
----------------

Once you've [enabled](serial-number#enable-serial-number.md) Serial Number, you can mark an item as a serialized item by first checking **Track Item** and then **Has Serial Number**
...
Once a Serial Number has been enabled for an [Item](items.md), transactions involving the Item that update Inventory (such as Stock Movements, Shipments, and Purchases) must include the Serial Number.
