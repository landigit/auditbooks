import require$$1$2, { contextBridge, ipcRenderer as ipcRenderer$1 } from "electron";
...
var IPC_MESSAGES = /* @__PURE__ */ ((IPC_MESSAGES2) => {
  IPC_MESSAGES2["OPEN_MENU"] = "open-menu";
  IPC_MESSAGES2["OPEN_SETTINGS"] = "open-settings";
  IPC_MESSAGES2["OPEN_EXTERNAL"] = "open-external";
  IPC_MESSAGES2["SHOW_ITEM_IN_FOLDER"] = "show-item-in-folder";
  IPC_MESSAGES2["RELOAD_MAIN_WINDOW"] = "reload-main-window";
  IPC_MESSAGES2["MINIMIZE_MAIN_WINDOW"] = "minimize-main-window";
  IPC_MESSAGES2["MAXIMIZE_MAIN_WINDOW"] = "maximize-main-window";
  IPC_MESSAGES2["ISMAXIMIZED_MAIN_WINDOW"] = "ismaximized-main-window";
  IPC_MESSAGES2["ISMAXIMIZED_RESULT"] = "ismaximized-result";
  IPC_MESSAGES2["ISFULLSCREEN_MAIN_WINDOW"] = "isfullscreen-main-window";
  IPC_MESSAGES2["ISFULLSCREEN_RESULT"] = "isfullscreen-result";
  IPC_MESSAGES2["CLOSE_MAIN_WINDOW"] = "close-main-window";
  return IPC_MESSAGES2;
})(IPC_MESSAGES || {});
var IPC_ACTIONS = /* @__PURE__ */ ((IPC_ACTIONS2) => {
  IPC_ACTIONS2["GET_OPEN_FILEPATH"] = "open-dialog";
  IPC_ACTIONS2["GET_SAVE_FILEPATH"] = "save-dialog";
  IPC_ACTIONS2["GET_DIALOG_RESPONSE"] = "show-message-box";
  IPC_ACTIONS2["GET_ENV"] = "get-env";
  IPC_ACTIONS2["SAVE_HTML_AS_PDF"] = "save-html-as-pdf";
  IPC_ACTIONS2["PRINT_HTML_DOCUMENT"] = "print-html-document";
  IPC_ACTIONS2["SAVE_DATA"] = "save-data";
  IPC_ACTIONS2["SHOW_ERROR"] = "show-error";
  IPC_ACTIONS2["SEND_ERROR"] = "send-error";
  IPC_ACTIONS2["GET_LANGUAGE_MAP"] = "get-language-map";
  IPC_ACTIONS2["CHECK_FOR_UPDATES"] = "check-for-updates";
  IPC_ACTIONS2["CHECK_DB_ACCESS"] = "check-db-access";
  IPC_ACTIONS2["SELECT_FILE"] = "select-file";
  IPC_ACTIONS2["GET_CREDS"] = "get-creds";
  IPC_ACTIONS2["GET_DB_LIST"] = "get-db-list";
  IPC_ACTIONS2["GET_TEMPLATES"] = "get-templates";
  IPC_ACTIONS2["INIT_SHEDULER"] = "init-scheduler";
  IPC_ACTIONS2["DELETE_FILE"] = "delete-file";
  IPC_ACTIONS2["GET_DB_DEFAULT_PATH"] = "get-db-default-path";
  IPC_ACTIONS2["SEND_API_REQUEST"] = "send-api-request";
  IPC_ACTIONS2["DB_CREATE"] = "db-create";
  IPC_ACTIONS2["DB_CONNECT"] = "db-connect";
  IPC_ACTIONS2["DB_CALL"] = "db-call";
  IPC_ACTIONS2["DB_BESPOKE"] = "db-bespoke";
  IPC_ACTIONS2["DB_SCHEMA"] = "db-schema";
  return IPC_ACTIONS2;
})(IPC_ACTIONS || {});
var IPC_CHANNELS = /* @__PURE__ */ ((IPC_CHANNELS2) => {
  IPC_CHANNELS2["TRIGGER_ERPNEXT_SYNC"] = "trigger-erpnext-sync";
  IPC_CHANNELS2["LOG_MAIN_PROCESS_ERROR"] = "main-process-error";
  IPC_CHANNELS2["CONSOLE_LOG"] = "console-log";
  return IPC_CHANNELS2;
})(IPC_CHANNELS || {});
const ipc = {
  desktop: true,
  reloadWindow() {
    return ipcRenderer$1.send(IPC_MESSAGES.RELOAD_MAIN_WINDOW);
  },
  minimizeWindow() {
    return ipcRenderer$1.send(IPC_MESSAGES.MINIMIZE_MAIN_WINDOW);
  },
  toggleMaximize() {
    return ipcRenderer$1.send(IPC_MESSAGES.MAXIMIZE_MAIN_WINDOW);
  },
  isMaximized() {
    return new Promise((resolve2) => {
      ipcRenderer$1.send(IPC_MESSAGES.ISMAXIMIZED_MAIN_WINDOW);
      ipcRenderer$1.once(
        IPC_MESSAGES.ISMAXIMIZED_RESULT,
        (_event, isMaximized) => {
          resolve2(isMaximized);
        }
      );
    });
  },
  isFullscreen() {
    return new Promise((resolve2) => {
      ipcRenderer$1.send(IPC_MESSAGES.ISFULLSCREEN_MAIN_WINDOW);
      ipcRenderer$1.once(
        IPC_MESSAGES.ISFULLSCREEN_RESULT,
        (_event, isFullscreen) => {
          resolve2(isFullscreen);
        }
      );
    });
  },
  closeWindow() {
    return ipcRenderer$1.send(IPC_MESSAGES.CLOSE_MAIN_WINDOW);
  },
  async getCreds() {
    return await ipcRenderer$1.invoke(IPC_ACTIONS.GET_CREDS);
  },
  async getLanguageMap(code2) {
    return await ipcRenderer$1.invoke(IPC_ACTIONS.GET_LANGUAGE_MAP, code2);
  },
  async getTemplates(posTemplateWidth) {
    return await ipcRenderer$1.invoke(
      IPC_ACTIONS.GET_TEMPLATES,
      posTemplateWidth
    );
  },
  async initScheduler(time) {
    await ipcRenderer$1.invoke(IPC_ACTIONS.INIT_SHEDULER, time);
  },
  ...
  async checkForUpdates() {
    await ipcRenderer$1.invoke(IPC_ACTIONS.CHECK_FOR_UPDATES);
  },
  openLink(link) {
    ipcRenderer$1.send(IPC_MESSAGES.OPEN_EXTERNAL, link);
  },
  ...
  async saveData(data, savePath) {
    await ipcRenderer$1.invoke(IPC_ACTIONS.SAVE_DATA, data, savePath);
  },
  showItemInFolder(filePath) {
    ipcRenderer$1.send(IPC_MESSAGES.SHOW_ITEM_IN_FOLDER, filePath);
  },
  ...
  async getEnv() {
    return await ipcRenderer$1.invoke(IPC_ACTIONS.GET_ENV);
  },
  openExternalUrl(url) {
    ipcRenderer$1.send(IPC_MESSAGES.OPEN_EXTERNAL, url);
  },
  async showError(title2, content) {
    await ipcRenderer$1.invoke(IPC_ACTIONS.SHOW_ERROR, { title: title2, content });
  },
  async sendError(body) {
    await ipcRenderer$1.invoke(IPC_ACTIONS.SEND_ERROR, body);
  },
  ...
  registerMainProcessErrorListener(listener) {
    ipcRenderer$1.on(IPC_CHANNELS.LOG_MAIN_PROCESS_ERROR, listener);
  },
  registerTriggerFrontendActionListener(listener) {
    ipcRenderer$1.on(IPC_CHANNELS.TRIGGER_ERPNEXT_SYNC, listener);
  },
  registerConsoleLogListener(listener) {
    ipcRenderer$1.on(IPC_CHANNELS.CONSOLE_LOG, listener);
  },
  db: {
    async getSchema() {
      return await ipcRenderer$1.invoke(
