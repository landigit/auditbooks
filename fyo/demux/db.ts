...
export class DatabaseDemux extends DatabaseDemuxBase {
	...
	async getSchemaMap(): Promise<SchemaMap> {
		...
		return (await this.#handleDBCall(async () => {
			return await window.auditbooksIpc.db.getSchema();
		})) as SchemaMap;
	}
	...
	async createNewDatabase(
		...
		return (await this.#handleDBCall(async () => {
			return window.auditbooksIpc.db.create(dbPath, countryCode);
		})) as string;
	}
	...
	async connectToDatabase(
		...
		return (await this.#handleDBCall(async () => {
			return window.auditbooksIpc.db.connect(dbPath, countryCode);
		})) as string;
	}
	...
	async call(method: DatabaseMethod, ...args: unknown[]): Promise<unknown> {
		...
		return await this.#handleDBCall(async () => {
			return await window.auditbooksIpc.db.call(method, ...args);
		});
	}
	...
	async callBespoke(method: string, ...args: unknown[]): Promise<unknown> {
		...
		return await this.#handleDBCall(async () => {
			return await window.auditbooksIpc.db.bespoke(method, ...args);
		});
	}
}
