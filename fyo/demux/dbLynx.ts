...
// ============================================================================
export class LynxDatabaseCore {
  client: NativeSqliteClient | any;
  schemaMap: SchemaMap = {};
  ...
  async get(
    ...
    const nonTableFields = fields.filter((f) => !tableFieldNames.includes(f));
  ...
  async getAll(
    ...
    if (orderBy) {
      ...
      sql += ` ORDER BY ${cols.map((c) => `"${c}"`).join(', ')} ${order.toUpperCase()}`;
    }
    if (limit !== undefined) {
      sql += ' LIMIT ?';
      args.push(limit);
  ...
  async getSingleValues(
    ...
    try {
      console.log(
        '[Debug dbLynx] getSingleValues SQL:',
      ...
      console.log(
        '[Debug dbLynx] getSingleValues SQL execute result:',
        JSON.stringify(r)
      );
      return (r?.rows ?? []) as any[];
    } catch (err) {
      console.error('[Debug dbLynx] getSingleValues SQL execute error:', err);
      return [];
    }
  }
  ...
  private async createTable(schemaName: string): Promise<void> {
    ...
    for (const field of schema.fields) {
      ...
      if (field.fieldtype === FieldTypeEnum.Link && field.target) {
        const target = this.schemaMap[field.target];
        if (target) {
          fks.push(
  ...
  private async getColumnDiff(
    ...
    const existing = r.rows.map((row: any) => row.name as string);
    const schema = this.schemaMap[schemaName];
    const added: Field[] = [];
