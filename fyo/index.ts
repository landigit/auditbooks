import { getMoneyMaker, type MoneyMaker } from "pesa";
...
export class Fyo {
	...
	constructor(conf: FyoConfig = {}) {
		...
		this.auth = markRaw(new AuthHandler(this, conf.AuthDemux));
		this.db = markRaw(new DatabaseHandler(this, conf.DatabaseDemux));
		this.doc = markRaw(new DocHandler(this));
		...
		this.telemetry = markRaw(new TelemetryManager(this));
		this.config = markRaw(new Config(this.isElectron && !this.isTest));
	}
	...
	setIsElectron() {
		try {
			this.isElectron = !!window?.auditbooksIpc;
		} catch {
