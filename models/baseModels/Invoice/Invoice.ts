...
export abstract class Invoice extends Transactional {
	...
	async getReturnDoc(): Promise<Invoice | undefined> {
		...
		for (const item of docItems) {
				...
				if (item.batch) {
					...
					if (typeof returnData === "object" && returnData?.batches) {
						returnDocItems = docItems.map((docItem: DocValueMap) => {
							const qty =
								-(returnData as any)?.batches?.[docItem.batch as string] ?? 0;
							const transferQty =
			...
			if (item.batch && returnedItem.batches?.[item.batch as string]) {
			...
			formula: async () => {
				...
				if (this.isReturn) {
					...
					if (sinvreturnedDoc.outstandingAmount?.isZero()) {
						...
						const gtotal = sinvreturnedDoc.grandTotal?.abs();
						const outstandingAmountSource =
							sinvreturnedDoc.outstandingAmount ?? this.fyo.pesa(0);
						const totalPaid = gtotal?.sub(outstandingAmountSource);

						const outstandingAmount = this.grandTotal?.abs();

						if (!outstandingAmount || !totalPaid) {
							return outstandingAmount || totalPaid;
						}
	...
	async getStockTransfer(isAuto = false): Promise<StockTransfer | null> {
		...
		let terms: string;
		let numberSeries: string | undefined;
	...
	async getLinkedStockTransfers() {
		...
		const transferItems = (await this.fyo.db.getAllRaw(itemSchemaName, {
			...
			filters: {
				...
				item: ["in", (this.items?.map((i) => i.item!) ?? []) as string[]],
			},
		})) as {
			parent: string;
			quantity: number;
