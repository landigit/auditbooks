...
export class SalesInvoice extends Invoice {
	...
	async getPosting() {
		...
		for (const item of this.items!) {
			const amount = (item.amount ?? this.fyo.pesa(0)).mul(exchangeRate);
			if (this.isReturn) {
		...
		if (this.taxes) {
			for (const tax of this.taxes) {
				const amount = (tax.amount ?? this.fyo.pesa(0)).mul(exchangeRate);
				if (this.isReturn) {
					await posting.debit(tax.account!, amount);
					continue;
				}
				await posting.credit(tax.account!, amount);
			}
		}
