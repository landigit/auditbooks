...
const { DateTime } = _require("luxon");
// Money is required as a value for `new Money(...)` calls
const MoneyClass = _require("pesa").Money as typeof import("pesa").Money;
...
import type { Item } from "./baseModels/Item/Item";
	...
	for (const row of loyaltyProgramData.collectionRules) {
		if (row.minimumTotalSpent !== undefined && row.minimumTotalSpent !== null) {
			...
			if (isPesa(row.minimumTotalSpent)) {
				...
				minimumSpent = new MoneyClass(row.minimumTotalSpent as number);
			}
...
export async function getPricingRule(
	...
	for (const item of doc.items) {
		...
		let pricingRuleDocsForItem: PricingRule[] | undefined;
