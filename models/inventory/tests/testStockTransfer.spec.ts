...
describe("Stock Transfer", () => {
  ...
  test("inventory settings", async () => {
    const doc = (await fyo.doc.getDoc(ModelNameEnum.InventorySettings)) as InventorySettings;
  ...
  test("Back Ref Purchase Receipt cancel", async () => {
    ...
    const pinv = (await fyo.doc.getDoc(ModelNameEnum.PurchaseInvoice, "PINV-1001")) as Invoice;
  ...
  test("Sales Invoice then another Shipment", async () => {
    const sinv = (await fyo.doc.getDoc(ModelNameEnum.SalesInvoice, "SINV-1001")) as Invoice;
  ...
  test("Cancel partial Shipment", async () => {
    let shpm = (await fyo.doc.getDoc(ModelNameEnum.Shipment, "SHPM-1003")) as StockTransfer;
    ...
    shpm = (await fyo.doc.getDoc(ModelNameEnum.Shipment, "SHPM-1004")) as StockTransfer;
  ...
  test("Duplicate Shipment, backref unset", async () => {
    const shpm = (await fyo.doc.getDoc(ModelNameEnum.Shipment, "SHPM-1003")) as StockTransfer;
  ...
  test("Create Shipment then create return against it", async (_t) => {
    ...
    const returnShpmAles = (await fyo.db.getAllRaw(ModelNameEnum.AccountingLedgerEntry, {
  ...
  test("Create Purchase Reciept then create return against it", async () => {
    const rate = testDocs.Item[item].rate as number;
    const prec = fyo.doc.getNewDoc(ModelNameEnum.PurchaseReceipt) as PurchaseReceipt;
    ...
    const returnPrecAles = (await fyo.db.getAllRaw(ModelNameEnum.AccountingLedgerEntry, {
      fields: ["name", "account", "credit", "debit"],
