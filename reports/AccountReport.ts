...
export abstract class AccountReport extends LedgerReport {
  ...
  getRootNodes(rootType: AccountRootType, accountTree: AccountTree): AccountTreeNode[] | undefined {
  ...
  async _getGroupedByDateRanges(map: GroupedMap): Promise<AccountNameValueMapMap> {
    ...
    for (const account of map.keys()) {
      ...
      for (const entry of map.get(account)!) {
        ...
        if (!this.accountMap || !Reflect.get(this.accountMap, entry.account)) {
        ...
        const rootType = Reflect.get(this.accountMap!, entry.account)?.rootType;
  ...
  async _getDateRanges(): Promise<DateRange[]> {
    ...
    const months: number = Reflect.get(monthsMap, this.periodicity);
    ...
    for (let i = 1; i < count; i++) {
      ...
      dateRanges.push({
        ...
        fromDate: this._fixMonthsJump(toDate, lastRange.fromDate.subtract(months, "month")),
      });
    }
    ...
    if (this.basedOn === "Until Date") {
      dateFilters = [
        ...
        },
        {
          ...
          fieldname: "count",
          minvalue: 1,
          placeholder: t`Number of ${Reflect.get(periodNameMap, this.periodicity)}`,
          label: t`Number of ${Reflect.get(periodNameMap, this.periodicity)}`,
          required: true,
        },
      ] as Field[];
    }
...
export async function getFiscalEndpoints(toYear: number, fromYear: number, fyo: Fyo) {
  const fys = (await fyo.getValue(ModelNameEnum.AccountingSettings, "fiscalYearStart")) as Date;
  const fye = (await fyo.getValue(ModelNameEnum.AccountingSettings, "fiscalYearEnd")) as Date;
  ...
  for (const key of valueMap.keys()) {
    ...
    if (childValue) {
      for (const key of Object.keys(childValue)) {
        ...
        const cvVal = childValue ? Reflect.get(childValue, key) : undefined;
        Reflect.set(map, key, (vVal ?? 0) + (cvVal ?? 0));
      }
    }
...
function setChildrenOnAccountTreeNodes(accountTree: AccountTree) {
  ...
  for (const name of Object.keys(accountTree)) {
    ...
    const parentNode = Reflect.get(accountTree, ac.parentAccount);
    parentNode.children ??= [];
    parentNode.children.push(ac);
...
function pruneAccountTree(accountTree: AccountTree) {
  for (const root of Object.keys(accountTree)) {
    ...
    if (node && node.prune) {
      Reflect.deleteProperty(accountTree, root);
    }
  }
...
export function convertAccountRootNodesToAccountList(rootNodes: AccountTreeNode[]): AccountList {
