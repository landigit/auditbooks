...
export const reports = {
  GeneralLedger: {
    ...
    load: () =>
      import('./GeneralLedger/GeneralLedger').then((m) => m.GeneralLedger),
  },
  ProfitAndLoss: {
    title: t`Profit and Loss`,
    reportName: 'profit-and-loss',
    load: () =>
      import('./ProfitAndLoss/ProfitAndLoss').then((m) => m.ProfitAndLoss),
  },
  BalanceSheet: {
    title: t`Balance Sheet`,
    reportName: 'balance-sheet',
    load: () =>
      import('./BalanceSheet/BalanceSheet').then((m) => m.BalanceSheet),
  },
  TrialBalance: {
    title: t`Trial Balance`,
    reportName: 'trial-balance',
    load: () =>
      import('./TrialBalance/TrialBalance').then((m) => m.TrialBalance),
  },
  GSTR1: {
    title: t`GSTR-1`,
    reportName: 'gstr-1',
