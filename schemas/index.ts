...
export function addMetaFields(schemaMap: SchemaMap): SchemaMap {
	const metaSchemaMap: SchemaStubMap = getMapFromList(
		cloneDeep(metaSchemas) as SchemaStub[],
		"name",
	);

	const base = metaSchemaMap.base ?? { name: "base" };
	const tree = getCombined(
		metaSchemaMap.tree ?? { name: "tree" },
		base as SchemaStub,
	);
	const child = metaSchemaMap.child ?? { name: "child" };
	const submittable = getCombined(
		metaSchemaMap.submittable ?? { name: "submittable" },
		base as SchemaStub,
	);
	const submittableTree = getCombined(
		tree,
		metaSchemaMap.submittable ?? { name: "submittable" },
	);
...
function getCoreSchemas(): SchemaMap {
	const rawSchemaMap = getMapFromList(
		cloneDeep(coreSchemas) as SchemaStub[],
		"name",
	);
	const coreSchemaMap = getAbstractCombinedSchemas(
		rawSchemaMap as SchemaStubMap,
	);
