...
async function adjustFile(
  ...
  // Group rows by payment name
  const paymentGroups: Record<string, string[][]> = {};
  ...
  for (const pName in paymentGroups) {
    ...
    for (const row of groupRows) {
      ...
      let allocAmt = parseFloat(row[allocAmtIdx] ?? '0') || 0;

      if (refName) {
        const inv = await client.execute({
          sql: `SELECT outstandingAmount FROM ${tableName} WHERE name = ?`,
          args: [refName],
        });
        const outstandingRaw = inv.rows[0]?.['outstandingAmount'];
        if (outstandingRaw !== null && outstandingRaw !== undefined) {
          const outstanding = Math.abs(parseFloat(String(outstandingRaw)));
          if (
