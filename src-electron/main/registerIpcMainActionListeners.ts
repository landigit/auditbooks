...
const databaseManager =
	require("backend/database/manager").default ||
	require("backend/database/manager");
const { emitMainProcessError } = require("backend/helpers");
const { IPC_ACTIONS } = require("utils/messages");
const { sendAPIRequest } = require("./api.ts");
const { getUrlAndTokenString, sendError } = require("./contactMothership.ts");
const { getLanguageMap } = require("./getLanguageMap.ts");
const { getTemplates } = require("./getPrintTemplates.ts");
const {
...
const { initScheduler } = require("./initSheduler.ts");
const { printHtmlDocument } = require("./printHtmlDocument.ts");
const { saveHtmlAsPdf } = require("./saveHtmlAsPdf.ts");

function registerIpcMainActionListeners(main: Main) {
	...
	ipcMain.handle("is-maximized", () => {
		return main.mainWindow?.isMaximized() ?? false;
	});

	ipcMain.handle("is-fullscreen", () => {
		return main.mainWindow?.isFullScreen() ?? false;
	});
}
module.exports = {
	registerIpcMainActionListeners,
	default: registerIpcMainActionListeners,
};
