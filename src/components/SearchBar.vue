<template>
  <div>
    ...
    <UIButton
      variant="outline"
      size="icon"
      class="rounded-r-none dark:bg-gray-900"
      @click="open"
    >
      <LucideIcon
        name="search"
  ...
  <Dialog :open="openModal" @update:open="openModal = $event">
    <DialogContent class="sm:max-w-2xl glass p-0 overflow-hidden">
      ...
      <DialogTitle class="sr-only">{{ t`Quick Search` }}</DialogTitle>
      ...
      <div class="w-full">
        ...
        <div class="p-2 flex justify-between flex-col gap-2 text-sm select-none">
          <!-- Group Filters -->
          <div class="flex justify-between">
            <div class="flex gap-1">
              <button
                ...
                :key="g"
                class="border dark:border-gray-800 px-1 py-0.5 rounded-md text-xs"
            ...
            <button
              class="hover:text-gray-900 dark:hover:text-gray-25 py-0.5 rounded text-gray-700 dark:text-gray-300 text-xs"
          ...
          <div v-if="showMore" class="-mt-1">
            <div class="flex gap-1 text-gray-800 dark:text-gray-200">
              <button
                ...
                :key="s"
                class="border dark:border-gray-800 px-1 py-0.5 rounded-md text-xs"
                :class="{ 'bg-gray-200 dark:bg-gray-800': searcher?.filters[s] }"
                @click="searcher?.set(s, !searcher?.filters[s])"
              >
                {{
                  s === 'skipTables' ? t`Skip Tables` : t`Skip Txns`
                }}
              </button>
            </div>
          </div>
          ...
          <!-- Keybindings Help -->
          <div class="flex text-xs text-gray-500 justify-between items-baseline pt-1">
            <div class="flex gap-3">
              ...
              >
                <LucideIcon name="help-circle" :size="12" class="w-3 h-3 me-1" />
                {{ t`Help` }}
              </button>
            </div>

            <p v-if="searcher?.numSearches" class="ms-auto">
              {{ suggestions.length }} / {{ searcher.numSearches }}
            </p>
          </div>
        </div>
      </div>
...
export default defineComponent({
  components: { Dialog, DialogContent, DialogTitle, LucideIcon },
  setup() {
    return {
