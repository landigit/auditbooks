...
watch(() => props.modelValue, (newVal) => {
  if (newVal) {
    datePart.value = new CalendarDate(newVal.getFullYear(), newVal.getMonth() + 1, newVal.getDate())
...
<template>
  <Popover>
    <PopoverTrigger as-child>
      ...
      >
        <div class="flex items-center gap-2 overflow-hidden whitespace-nowrap text-sm">
    ...
    <PopoverContent class="w-auto p-0 overflow-hidden shadow-lg border-border">
      <div class="flex flex-col sm:flex-row divide-y sm:divide-y-0 sm:divide-x divide-border">
        <Calendar 
        ...
        <div class="p-4 flex flex-col gap-4 bg-surface min-w-[120px]">
          <div class="flex flex-col gap-1.5">
            <label class="text-xs font-medium text-description uppercase tracking-wider">{{ t`Time` }}</label>
            <div class="flex items-center gap-2">
              <div class="flex flex-col gap-1">
                <input
                  ...
                  @input="e => handleTimeChange('hours', (e.target as HTMLInputElement).value)"
                />
                <span class="text-[10px] text-description text-center uppercase">{{ t`Hrs` }}</span>
              </div>
              ...
              <div class="flex flex-col gap-1">
                <input
                  ...
                  class="w-12 h-9 text-center bg-surface border border-border rounded text-sm focus:ring-2 focus:ring-indicator-green-bg outline-none"
                  @input="e => handleTimeChange('minutes', (e.target as HTMLInputElement).value)"
                />
                <span class="text-[10px] text-description text-center uppercase">{{ t`Min` }}</span>
              </div>
            </div>
          </div>
