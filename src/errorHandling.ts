...
export async function sendError(errorLogObj: ErrorLog) {
	...
	await window.auditbooksIpc.sendError(JSON.stringify(body));
}
...
export async function showErrorDialog(title?: string, content?: string) {
	...
	await window.auditbooksIpc.showError(title, content);
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function getErrorHandled<
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function getErrorHandledSync<T extends (...args: unknown[]) => unknown>(
	func: T,
) {
...
export function reportIssue(errorLogObj?: ErrorLog) {
	...
	window.auditbooksIpc.openExternalUrl(urlQuery);
}
