...
const dateValue = ref<DateValue>(new CalendarDate(today.getFullYear(), today.getMonth() + 1, today.getDate()))
...
async function fetchTransactions() {
  ...
  const allResults = await Promise.all(schemas.map(schema => 
  ...
  transactions.value = allResults.flat().sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
}

const selectedDayTransactions = computed(() => {
  if (!dateValue.value) return []
  const selectedDateStr = new Date(dateValue.value.year, dateValue.value.month - 1, dateValue.value.day).toISOString().split('T')[0]
...
<template>
  <div class="h-screen flex flex-col bg-canvas">
    ...
    <div class="flex-1 flex overflow-hidden p-6 gap-6">
      ...
      <div class="flex-1 bg-surface rounded border border-border flex flex-col overflow-hidden">
        <div class="p-4 border-b border-border flex justify-between items-center bg-surface-hover/30">
          <h2 class="font-semibold text-main text-lg">
            {{ dateValue ? new Date(dateValue.year, dateValue.month - 1, dateValue.day).toLocaleDateString(undefined, { dateStyle: 'full' }) : '' }}
          </h2>
          <Badge class="bg-indicator-blue-bg text-indicator-blue-text border-indicator-blue-text border">
        ...
        <div class="flex-1 overflow-auto p-4 space-y-3 no-scrollbar">
          ...
          >
            <div class="flex flex-col">
              <span class="text-xs uppercase tracking-wider text-description font-semibold">{{ t.schemaName }}</span>
              <span class="font-medium text-main">{{ t.name }}</span>
              <span class="text-sm text-description">{{ t.party || t.reference_name || '' }}</span>
            </div>
            <div class="text-right flex flex-col items-end">
              <span class="font-bold text-main">{{ fyo.format(t.grandTotal || t.total_amount || 0, 'Currency') }}</span>
              <div class="mt-1">
          ...
          <div v-if="selectedDayTransactions.length === 0" class="h-full flex flex-col items-center justify-center text-description py-20">
