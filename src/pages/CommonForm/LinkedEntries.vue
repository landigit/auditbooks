  ...
  <view v-else class="MainView">
    ...
    <view class="NavBar">
      <view class="flex-row items-center">
        ...
        <view class="NavBrand">
          <text class="BrandText">{{ t`Linked Entries` }}</text>
        </view>
      </view>
    </view>
    ...
    >
      ...
      <!-- Linked Entry List -->
      <view v-else-if="sequence.length" class="p-4">
        <view
          v-for="sn of sequence"
          :key="sn"
          class="mb-4 bg-canvas rounded-xl p-3 border border-border"
        >
          <!-- Header with count and schema label -->
          <view
            class="flex-row justify-between items-center py-2"
            @tap="entries[sn].collapsed = !entries[sn].collapsed"
          >
          ...
          <!-- Entry list -->
          <view v-if="!entries[sn].collapsed" class="mt-2 flex-col gap-2">
            <!-- Entry Card -->
            <view
              v-for="e of entries[sn].details"
              :key="String(e.name) + sn"
              class="p-3 bg-canvas-muted rounded-lg border border-border"
              @tap="routeToEntry(sn, String(e.name))"
            >
              ...
              <!-- Values summary line -->
              <view class="flex-row flex-wrap gap-2 mt-1">
                <!-- Credit or Debit (GLE) -->
                <text
                  v-if="isPesa(e.credit) && e.credit.isPositive()"
                ...
                <text
                  v-if="e.party || e.entryType || e.account"
                  class="text-xs px-2 py-0.5 rounded bg-neutral-muted text-description"
                >
                  {{ e.party || e.entryType || e.account }}
                </text>
      ...
      <view
        v-else
        class="flex flex-col items-center justify-center p-12 bg-surface rounded-2xl border border-border m-4 shadow-sm"
      >
        <view
        ...
        <text class="text-xs text-description text-center">
          {{ t`No related documents found for this entry.` }}
        </text>
      </view>
    </scroll-view>
  </view>
