<template>
  <div class="flex flex-col flex-1 h-full bg-gray-25 dark:bg-gray-875 overflow-hidden">
    <PageHeader :border="true" :title="t`Invoice Designer`">
      ...
      <Button type="primary" class="text-xs" @click="download" :disabled="!values">
        <feather-icon name="download" class="w-4 h-4 md:me-1.5" />
        <span class="hidden md:inline">{{ t`Download PDF` }}</span>
      </Button>
      <Button type="primary" class="text-xs" @click="doPrint" :disabled="!values">
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      >
        ...
        <div class="p-3 border-b dark:border-gray-800">
          <p class="text-[10px] font-semibold text-gray-500 uppercase tracking-widest mb-2">
            {{ t`Invoice Style` }}
          </p>
          <div class="grid grid-cols-2 gap-1.5">
            ...
            >
              ...
              />
              <span class="font-semibold text-gray-800 dark:text-gray-200">{{ preset.label }}</span>
              <p class="text-[9px] text-gray-400 mt-0.5 leading-tight">{{ preset.description }}</p>
            </button>
          </div>
        </div>
        ...
        <div class="p-3 border-b dark:border-gray-800">
          ...
          <div class="flex items-center gap-2">
            <input type="checkbox" id="showPageNumbers" v-model="showPageNumbers" class="rounded border-gray-350 dark:border-gray-700 text-blue-600 focus:ring-blue-500" />
        ...
        <div class="p-3 flex flex-col gap-2">
          <p class="text-[10px] font-semibold text-gray-500 uppercase tracking-widest mb-1">
      ...
      <!-- ── Right: live preview ── -->
      <main class="flex-1 overflow-y-auto bg-gray-100 dark:bg-gray-900 p-6 custom-scroll">
        ...
        <template v-else>
          ...
          <div
            ...
            style="width: 794px; min-height: 1123px; padding: 48px; box-sizing: border-box; position: relative; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #000; line-height: 1.3;"
          >
            <!-- ── Header ── -->
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:24px;">
              <div>
                <img
                  ...
                  :src="printData.logo"
                />
                <div v-else style="font-size:1.25rem; font-weight:bold; color:#000;">
                  {{ printData.companyName }}
                </div>
              </div>
              <div style="text-align:right;">
                <h1 style="font-size:1.25rem; font-weight:bold; margin:0;">{{ docData.entryLabel || t`Tax Invoice/Bill of Supply` }}</h1>
                <p style="font-size:0.72rem; color:#6b7280; margin:2px 0 0 0;">(Original for Recipient)</p>
              </div>
            </div>

            <!-- ── Address Section ── -->
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:32px; margin-bottom:16px; font-size:0.75rem;">
              <!-- Left Column: Sold By -->
              <div>
                ...
                <div style="margin-top:6px; font-size:0.7rem; color:#374151;">
                  <p v-if="printData.pan" style="margin:0;"><span style="font-weight:bold;">PAN No:</span> {{ printData.pan }}</p>
                  <p v-if="printData.gstin" style="margin:2px 0 0 0;"><span style="font-weight:bold;">GST Registration No:</span> {{ printData.gstin }}</p>
                </div>
              </div>
              <!-- Right Column: Billing Address -->
              <div style="text-align:right;">
                <h2 style="font-weight:bold; margin:0 0 4px 0;">Billing Address :</h2>
                <div style="white-space:pre-line; color:#1f2937;">
                  {{ docData.party }}
                  {{ docData.links?.party?.links?.address?.addressDisplay || docData.links?.party?.address || '' }}
                </div>
                <div v-if="docData.partyGSTIN" style="margin-top:6px; font-size:0.7rem; color:#374151;">
                  <p style="margin:0;"><span style="font-weight:bold;">GSTIN:</span> {{ docData.partyGSTIN }}</p>
                </div>
              </div>
            </div>

            <!-- ── Shipping Address ── -->
            <div style="display:flex; flex-direction:column; align-items:flex-end; margin-bottom:20px; font-size:0.75rem;">
              <div style="text-align:right;">
                <h2 style="font-weight:bold; margin:0 0 4px 0;">Shipping Address :</h2>
                <div style="white-space:pre-line; color:#1f2937;">
                  {{ docData.party }}
                  {{ docData.links?.shippingAddress?.addressDisplay || docData.links?.party?.links?.address?.addressDisplay || docData.links?.party?.address || '' }}
                </div>
              </div>
            </div>

            <!-- ── Meta Information ── -->
            <div style="display:flex; justify-content:space-between; align-items:flex-end; margin-bottom:12px; font-size:0.72rem; border-bottom:1px solid #e5e7eb; padding-bottom:8px;">
              <div>
                <p v-if="docData.orderNumber" style="margin:0;"><span style="font-weight:bold;">Order Number:</span> {{ docData.orderNumber }}</p>
                <p style="margin:2px 0 0 0;"><span style="font-weight:bold;">Order Date:</span> {{ docData.date }}</p>
              </div>
              <div style="text-align:right;">
                <p style="margin:0;"><span style="font-weight:bold;">Invoice Number :</span> {{ docData.name }}</p>
                <p style="margin:2px 0 0 0;"><span style="font-weight:bold;">Invoice Date :</span> {{ docData.date }}</p>
              </div>
            </div>

            <!-- ── Items Table ── -->
            <table style="width:100%; border-collapse:collapse; font-size:0.72rem; margin-bottom:12px; border:1px solid #000;">
              <thead>
                <tr style="background:#f3f4f6; border-bottom:1px solid #000;">
                  <th
                    v-for="col in visibleColumns"
                    :key="col.fieldname"
                    style="padding:6px 8px; font-weight:bold; border-right:1px solid #000; color:#000;"
              ...
              <tbody>
                ...
                >
                  <td
                    ...
                    :style="{ textAlign: col.align }"
                  >{{ col.fieldname === 'idx' ? idx + 1 : row[col.fieldname] ?? '' }}</td>
                </tr>
              </tbody>
            </table>
            ...
            <div style="display:flex; justify-content:flex-end; margin-bottom:16px;">
              <div style="width:220px; font-size:0.72rem;">
                <div style="display:flex; justify-content:space-between; color:#4b5563; margin-bottom:3px;">
                  <span>{{ t`Total` }}</span><span>{{ docData.netTotal }}</span>
                </div>
                <div
                  v-for="tax in docTaxes"
                  :key="tax.account"
                  style="display:flex; justify-content:space-between; color:#4b5563; margin-bottom:2px;"
                >
                  <span>{{ tax.account }}</span><span>{{ tax.amount }}</span>
                </div>
                <div
                ...
                <div
                  style="display:flex; justify-content:space-between; font-weight:bold; font-size:0.85rem; border-top:1px solid #e5e7eb; padding-top:6px; margin-top:4px; color:#000;"
                >
                  <span>{{ t`Grand Total` }}</span><span>{{ printData.roundOffGrandTotal ? docData.roundedGrandTotal : docData.grandTotal }}</span>
                </div>
              </div>
            </div>

            <!-- ── Amount in Words ── -->
            <div v-if="printData.amountInWords && docData.grandTotalInWords" style="border:1px solid #000; padding:8px; margin-bottom:16px; font-size:0.72rem;">
              <p style="font-weight:bold; margin:0 0 2px 0;">Amount in Words:</p>
              <p style="margin:0; color:#374151;">{{ docData.grandTotalInWords }}</p>
            </div>
            ...
            <div v-if="showPageNumbers" class="absolute left-0 right-0 top-0 bottom-0 pointer-events-none" style="overflow: hidden;">
              <template v-for="p in pageCount" :key="p">
                ...
                >
                  <span class="bg-gray-100 px-2 py-0.5 text-[9px] text-gray-500 rounded border border-gray-300 translate-y-[-50%] pointer-events-auto">
                    {{ t`Page Break` }}
                  </span>
                </div>
                <div
          ...
          <div
            ...
            style="width: 320px; padding: 16px; font-family: 'Figtree', sans-serif; color: #000; font-size: 13px;"
          >
            <!-- Logo -->
            <div v-if="printData.displayLogo && printData.logo" style="text-align: center; margin-bottom: 8px;">
              <img :src="printData.logo" style="max-width: 100px; height: auto; display: inline-block;" />
            </div>
            
            <!-- Company Name -->
            <h2 style="font-weight: 600; text-align: center; font-size: 16px; margin: 4px 0;" :style="{ color: printData.color || '#000' }">
              {{ printData.companyName }}
            </h2>
            
            <!-- Address -->
            <p v-if="printData.address" style="text-align: center; margin: 2px 0;">
              {{ printData.links?.address?.addressDisplay || printData.address }}
            </p>
            
            <!-- Phone & Email -->
            <p style="text-align: center; margin: 2px 0;">{{ printData.phone }}</p>
            <p style="text-align: center; margin: 2px 0;">{{ printData.email }}</p>
            
            <!-- Title -->
            <h4 style="font-weight: 600; text-align: center; font-size: 16px; margin: 12px 0 6px 0;">Invoice</h4>
            ...
            <table style="width: 100%; font-size: 12px; border-collapse: collapse; margin-bottom: 8px;">
              <thead>
                <tr style="border-top: 1px solid #000; border-bottom: 1px solid #000; font-weight: bold;">
                  <th style="text-align: left; padding: 4px 0; width: 20px;">{{ t`SI` }}</th>
                  <th style="text-align: left; padding: 4px 0;">{{ t`Item` }}</th>
                  <th style="text-align: left; padding: 4px 0; width: 30px;">{{ t`Qty` }}</th>
                  <th style="text-align: right; padding: 4px 0; width: 60px;">{{ t`Price` }}</th>
                  <th style="text-align: right; padding: 4px 0; width: 60px;">{{ t`Amount` }}</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(row, idx) in docItems" :key="String(row.name ?? idx)">
                  <td style="padding: 4px 0;">{{ idx + 1 }}</td>
                  <td style="padding: 4px 0;">{{ row.item }}</td>
                  <td style="padding: 4px 0;">{{ row.quantity ?? row.qty }}</td>
                  <td style="text-align: right; padding: 4px 0;">{{ row.rate }}</td>
                  <td style="text-align: right; padding: 4px 0;">{{ row.amount }}</td>
                </tr>
              </tbody>
            </table>
            
            <!-- Totals row -->
            <div style="border-top: 1px solid #000; border-bottom: 1px solid #000; display: flex; justify-content: space-between; padding: 4px 0; font-size: 12px; margin-bottom: 8px;">
              <span>Total</span>
              <span>{{ docData.netTotal }}</span>
            </div>
            
            <!-- Net Total To Pay -->
            <div style="display: flex; justify-content: space-between; font-weight: bold; font-size: 15px; margin-bottom: 12px;">
              <span>Net Total To Pay</span>
              <span>{{ docData.grandTotal }}</span>
            </div>
            
            <!-- Tax breakdown -->
            <div v-if="docTaxes.length" style="border-top: 1px solid #000; padding-top: 6px; margin-bottom: 12px; font-size: 12px;">
              <p style="text-align: center; font-weight: bold; font-size: 14px; margin: 0 0 6px 0;">Tax Summary</p>
              <div style="display: flex; justify-content: space-between; padding-left: 10px;">
                <span>Total Ex.Tax</span>
                <span>{{ docData.subTotal }}</span>
              </div>
              <div v-for="tax in docTaxes" :key="tax.account" style="display: flex; justify-content: space-between; padding-left: 10px;">
            ...
            <!-- Payment breakdown -->
            <div style="border-top: 1px solid #000; border-bottom: 1px solid #000; padding: 6px 0; font-size: 12px; margin-bottom: 12px;">
              <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; font-weight: bold; margin-bottom: 4px;">
                <span>{{ t`Payment` }}</span>
                <span style="text-align: right;">{{ t`Tendered` }}</span>
                <span style="text-align: right;">{{ t`Balance` }}</span>
              </div>
              <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; border-top: 1px dashed #ccc; padding-top: 4px;">
                <span>{{ t`Discount` }}</span>
                <span style="text-align: right;">{{ docData.totalDiscount ? docData.totalDiscount : '00.00' }}</span>
                <span></span>
              </div>
              <div v-for="row in docData.paymentDetails" :key="row.paymentMethod" style="display: grid; grid-template-columns: 1fr 1fr 1fr;">
                <span>{{ row.paymentMethod }}</span>
                <span style="text-align: right;">{{ row.amount }}</span>
                <span style="text-align: right;">{{ row.outstandingAmount }}</span>
              </div>
            </div>
          ...
          <div
            ...
            ref="previewContainer"
            class="bg-white mx-auto rounded-none shadow-md"
            style="width: 794px; min-height: 1123px; padding: 48px; box-sizing: border-box; position: relative;"
          >
            <!-- ── Header ── -->
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:12px;">
              <div>
                ...
                >{{ printData.companyName }}</div>
                <div style="font-size:0.7rem; color:#9ca3af; margin-top:2px;">{{ printData.address }}</div>
                <div v-if="printData.gstin" style="font-size:0.7rem; color:#9ca3af;">GST: {{ printData.gstin }}</div>
              </div>
              <div style="text-align:right;">
                <span
                  style="display:inline-block; padding:4px 14px; border-radius:6px; font-size:0.85rem; font-weight:700; color:#fff;"
                  :style="{ background: currentPreset.primaryColor }"
                >{{ docData.entryLabel || 'TAX INVOICE' }}</span>
                <div style="font-size:0.72rem; color:#6b7280; margin-top:4px;"># {{ docData.name }}</div>
                <div style="font-size:0.7rem; color:#9ca3af;">{{ docData.date }}</div>
              </div>
            </div>
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:16px;">
              <div style="font-size:0.72rem;">
                <div
                  style="font-weight:700; text-transform:uppercase; font-size:0.62rem; letter-spacing:0.08em; margin-bottom:3px;"
                  :style="{ color: currentPreset.primaryColor }"
                >{{ t`Sold By` }}</div>
                <div style="font-weight:600;">{{ printData.companyName }}</div>
                <div style="color:#6b7280;">{{ printData.gstin ? `GSTIN: ${printData.gstin}` : '' }}</div>
              </div>
              <div style="font-size:0.72rem; text-align:right;">
                ...
                <div style="color:#6b7280;">{{ docData.partyGSTIN ? `GSTIN: ${docData.partyGSTIN}` : '' }}</div>
              </div>
            </div>
              ...
              <thead>
                <tr :style="{ background: currentPreset.primaryColor }">
                  <th
                    ...
                    :style="{ color: currentPreset.headerTextColor, textAlign: col.align }"
                  >{{ col.label }}</th>
                </tr>
              </thead>
              <tbody>
            ...
            <div style="display:flex; justify-content:flex-end; margin-bottom:12px;">
              <div style="width:220px; font-size:0.72rem;">
                ...
                <div
                  v-for="tax in docTaxes"
                  :key="tax.account"
                  style="display:flex; justify-content:space-between; color:#9ca3af; margin-bottom:2px;"
                >
                  <span>{{ tax.account }}</span><span>{{ tax.amount }}</span>
                </div>
                <div
                  v-if="printData.roundOffGrandTotal && docData.hasRoundOff"
                  style="display:flex; justify-content:space-between; color:#6b7280; margin-bottom:2px;"
                >
                  <span>Round Off</span><span>{{ docData.roundOff }}</span>
                </div>
                <div
                  style="display:flex; justify-content:space-between; font-weight:700; font-size:0.85rem; border-top:1px solid #e5e7eb; padding-top:6px; margin-top:4px;"
                  :style="{ color: currentPreset.primaryColor }"
                >
                  <span>{{ t`Grand Total` }}</span><span>{{ printData.roundOffGrandTotal ? docData.roundedGrandTotal : docData.grandTotal }}</span>
                </div>
            ...
            <!-- ── Amount in words ── -->
            <div v-if="printData.amountInWords && docData.grandTotalInWords" style="font-size:0.7rem; color:#6b7280; margin-top:4px;">
              <span style="font-weight:600;">{{ t`Amount in Words` }}: </span>{{ docData.grandTotalInWords }}
            </div>
...
const usedFieldnames = computed(() => new Set(columns.value.map((c) => c.fieldname)));
...
watch([columns, selectedStyle, values], () => {
...
const docTaxes = computed(
  () => (docData.value.taxes as { account: string; amount: string }[] | undefined) ?? []
);
...
// ── drag reorder ──
function onDragStart(idx: number) { dragging.value = idx; }
function onDragOver(idx: number) { dragOverIdx = idx; }
function onDrop() {
...
// ── actions ──
function resetColumns() { columns.value = DEFAULT_COLUMNS.map((c) => ({ ...c })); }
function addField(f: ColumnDef) { columns.value.push({ ...f, visible: true }); }
