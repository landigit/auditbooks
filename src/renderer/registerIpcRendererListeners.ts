...
export default function registerIpcRendererListeners() {
	window.auditbooksIpc.registerMainProcessErrorListener(
	...
	window.auditbooksIpc.registerTriggerFrontendActionListener(async () => {
		await syncDocumentsToERPNext(fyo);
	});

	window.auditbooksIpc.registerConsoleLogListener((_, ...stuff: unknown[]) => {
