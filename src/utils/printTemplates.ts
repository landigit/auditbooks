...
export async function getPrintTemplatePropValues(
	...
	let totalTax: Money | number | undefined;
...
function getGrandTotalInWords(total: number) {
	...
	const decimalCents = parseInt(decimalPart, 10);
...
export async function getPathAndMakePDF(
	...
	if (!shouldPrint) {
		...
		const success = await window.auditbooksIpc.makePDF(
			html,
			savePath,
			width,
			height,
		...
		const html = constructPrintDocument(innerHTML);
		const success = await window.auditbooksIpc.printDocument(
			html,
			width,
			height,
		);
...
export async function updatePrintTemplates(fyo: Fyo) {
	const templateFiles = await window.auditbooksIpc.getTemplates(
