...
export function showExportInFolder(message: string, filePath: string) {
	showToast({
		...
		type: "success",
		action: () => {
			window.auditbooksIpc.showItemInFolder(filePath);
		},
	});
}

export async function deleteDb(filePath: string) {
	const { error } = await window.auditbooksIpc.deleteFile(filePath);
...
export async function getSelectedFilePath() {
	return window.auditbooksIpc.getOpenFilePath({
		title: t`Select file`,
		properties: ["openFile"],
...
export async function getSavePath(name: string, extention: string) {
	const response = await window.auditbooksIpc.getSaveFilePath({
